...
\gls{exposuremodel} that needs to be provided in the NRML format. The
information included in an \gls{exposuremodel} comprises a metadata section listing
general information about the exposure, followed by a cost conversions section
that describes how the different areas, costs, and occupancies for the assets
will be specified, followed by data regarding each individual asset in the
portfolio.

A minimal \gls{exposuremodel} comprising a single asset is shown in
...
\begin{itemize}
  ...
  \item \Verb+id+: a unique key used to identify the \gls{exposuremodel}
...
The NRML schema for the \gls{exposuremodel} allows the definition of a
...
the units for the area of an asset. The \glsdesc{acr:oqe} itself is agnostic to the
area units; the \Verb+unit+ is thus a descriptive attribute which is used by the
\glsdesc{acr:oqe} to annotate the results of a risk assessment. This attribute
...
\begin{itemize}
  ...
  \item \Verb+id+: mandatory; a unique key used to identify the given
    \gls{asset}, which is used by the \glsdesc{acr:oqe} to relate each asset 
...
information needs to be stored in the \gls{exposuremodel} file, as shown in
...
engine is capable of employing the appropriate \gls{vulnerabilityfunction} or
\gls{fragilityfunction} in the risk calculations. Finally, the cost values of
...
such as \Verb+number+ or \Verb+area+. This mode of representing an
\gls{exposuremodel} is probably the simplest one.


\paragraph{Example 2}

In the snippet shown in Listing~\ref{lst:input_exposure_cunit_metadata}, an
\gls{exposuremodel} containing the number of units (buildings) and the
...
in which the economic cost of each asset is required, the \glsdesc{acr:oqe}
...
Once again, the \glsdesc{acr:oqe} needs to carry out some calculations in order
...
metadata section of an \gls{exposuremodel} where the insurance \glspl{limit}
and \glspl{deductible} for structural components will be defined relative to
the structural replacement cost.
...
\Verb+isAbsolute+ to \Verb+false+. Then, for each type of cost, the limit and
deductible value can be stored for each asset, as illustrated in the snippet
shown in Listing~\ref{lst:input_exposure_ins_rel_assets}.
...
to specify the sites where the hazard is computed; then any other exposure
model provided in a subsequent risk calculation which uses this hazard
calculation will be  ignored. In such cases, the risk results will be computed
based on the \gls{exposuremodel} previously specified in the hazard
calculation.



Scripts to convert an \gls{exposuremodel} in CSV format or as Excel or
